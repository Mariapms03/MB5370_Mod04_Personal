...
```{r, echo=FALSE}
## install.packages("tidyverse") # install the package, is necessary to do it only once
library("tidyverse") #load the package 
```

# Load the data (mpg is built into ggplot2)
For some explanation on the data, the displ = engine size od the cars in liters and hwy = car's fuel efficiency in miles per galon (mpg)
...
# Quick data checks
Entender que siginfica cada cosa de estas. 
```{r}
head(mpg)
glimpse(mpg)
summary(mpg)
...
# Create first plot 
We created a scatter plot with ggplot2 (point graph) with the `geom_point` layer. 
```{r}
ggplot(data = mpg) + 
  geom_point(mapping = aes(x = displ, y = hwy)) #geom_point is the point layer for the scatter plot
```
...
ANS: it just created the box for the plot but needs pecific parameters to create the graph and a dataset
```{r}
ggplot() 
...
# When you’re creating a plot, you essentially need two attributes of a plot: a geom and aesthetics (the size, shape and color).
```{r}
ggplot(data = mpg) + 
...
The warning is suggesting me that using size for a discrete variable might not be the beset way to get my result. Is no the best way to represent class in this data-set. 
...
ANS: would not be appropriate for the type of variable that class is because is not continuos. 
...
```
Also a warning saying that the shapes can only be used for 6 discrite classes and also that the data has some missing values and this ones weren't plot. 
...
Also the faced grid devides the plots by the variable we decide and puts them next to each other. Just one variable.
```{r}
ggplot(data = mpg) + 
...
Also the faced grid devides the plots and puts them next to each other. For more than one variable use facet_grid
```{r}
ggplot(data = mpg) + 
  geom_point(mapping = aes(x = displ, y = hwy)) + 
  facet_grid(drv ~ cyl)
...
```{r}
#ANS 
#this function is useful for creating multi-panel plots and also using different subsets of the data. 

#nrow: this is for specifying the number of rows in which the panels will be arranged. 

#ncol: same but number of columns
...
ANS 1: we do not use it because it would 
ANS 2:
